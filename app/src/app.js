...
export function pages({ register, post, gql, postWebpage, load }) {
  ...
  async function create(page, notify) {
    ...
    return Async.of(page)
      ...
      .chain(page =>
        Async.of(page).map(({ title, owner, description, html, theme }) => ({
          title,
          html: htmlTemplate(title, owner, description, html, theme)
...
function htmlTemplate(title, owner, description, body, theme="default") {
